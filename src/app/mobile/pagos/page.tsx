...
export default function MobilePaymentsPage() {
  ...
  const tabParam = searchParams?.get('tab') as 'realizados' | 'pendientes' | null

  const { user } = useAuth()
  const { logoUrl, logoMobileUrl } = useWhiteLabel()
  const customLogoUrl = logoMobileUrl || logoUrl || null
  
  const [activeTab, setActiveTab] = useState<'realizados' | 'pendientes'>(tabParam || 'realizados')

  useEffect(() => {
    if (tabParam && ['realizados', 'pendientes'].includes(tabParam)) {
      setActiveTab(tabParam)
    }
  }, [tabParam])

  const handleTabSelect = (tab: 'realizados' | 'pendientes') => {
    setActiveTab(tab)
    router.replace(`/mobile/pagos?tab=${tab}`, { scroll: false })
  }
    ...
    <div className="min-h-screen bg-[#FDFDFD] font-sans pb-8">
      ...
      <div className="px-6 mb-6 mt-4">
        <div className="flex bg-gray-100 p-1 rounded-2xl">
          <button 
            onClick={() => handleTabSelect('realizados')}
          ...
          <button 
            onClick={() => handleTabSelect('pendientes')}
