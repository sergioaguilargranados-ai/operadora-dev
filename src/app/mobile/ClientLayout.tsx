          ...
          <main className="flex-1 overflow-y-auto pb-16 flex flex-col">
            ...
            {isAuthenticated && !isLoginPage && (
              <div className="w-full text-center py-4 opacity-70">
                <span className="text-[8px] text-gray-400">v2.468 | 13 Aug 2026 21:54 CST | AS Operadora viajes y eventos</span>
              </div>
            )}
          </main>
          ...
          {isAuthenticated && !isLoginPage && (
            <nav className="fixed bottom-0 max-w-md mx-auto w-full bg-white/80 backdrop-blur-lg border-t border-gray-200 z-50 flex flex-col justify-between pt-2 pb-2">
              <div className="flex justify-around items-center px-4 w-full">
                <Link href="/mobile" className={`flex flex-col items-center gap-1 ${pathname === "/mobile" ? "text-brand-primary font-semibold" : "text-gray-400"}`}>
                  <Home className="w-5 h-5" />
                  <span className="text-[10px]">Inicio</span>
                </Link>
                <Link href="/mobile/itinerario" className={`flex flex-col items-center gap-1 ${pathname.startsWith("/mobile/itinerario") ? "text-brand-primary font-semibold" : "text-gray-400"}`}>
                  <Briefcase className="w-5 h-5" />
                  <span className="text-[10px]">Mis viajes</span>
                </Link>
                <Link href="/mobile/wishlist" className={`flex flex-col items-center gap-1 ${pathname.startsWith("/mobile/wishlist") ? "text-brand-primary font-semibold" : "text-gray-400"}`}>
                  <Heart className="w-5 h-5" />
                  <span className="text-[10px]">Wishlist</span>
                </Link>
                <Link href="/mobile/mapa" className={`flex flex-col items-center gap-1 ${pathname.startsWith("/mobile/mapa") ? "text-brand-primary font-semibold" : "text-gray-400"}`}>
                  <MapPin className="w-5 h-5" />
                  <span className="text-[10px]">Mapa</span>
                </Link>
                <Link href="/mobile/rewards" className={`flex flex-col items-center gap-1 ${pathname.startsWith("/mobile/rewards") ? "text-brand-primary font-semibold" : "text-gray-400"}`}>
