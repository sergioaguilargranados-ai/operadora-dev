...
export default function MobileMapPage() {
  ...
  // Geocode URL parameter `q` if provided (e.g. ?q=Tour por el Palacio Real)
  useEffect(() => {
      ...
      geocoder.geocode({ address: queryParam }, (results, status) => {
        if (status === 'OK' && results && results[0]) {
          ...
          const newPlace: Place = {
            ...
            lat: loc.lat(),
            lng: loc.lng(),
            desc: results[0].formatted_address || queryParam,
            icon: Landmark
          }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100 relative">
            ...
            {directionsResponse && (
              <DirectionsRenderer
                ...
                options={{
                  ...
                  polylineOptions: {
                    strokeColor: primaryColor || '#0066FF',
                    strokeWeight: 5,
                    strokeOpacity: 0.8,
                  },
                }}
      ...
      <div className={`fixed bottom-0 left-0 right-0 bg-white rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.15)] border-t z-30 transition-all duration-300 ease-in-out ${isDrawerExpanded ? 'p-5 pb-safe' : 'p-3 pb-10'}`}>
        <div 
