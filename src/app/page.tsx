...
export default function Home() {
  ...
  const handleSearchHotels = async () => {
    ...
    if (response && response.success) {
      router.push(`/resultados?type=hotel&data=${encodeURIComponent(JSON.stringify(response))}`)
    }
  }
  ...
  const handleSearchFlights = async () => {
    ...
    if (response && response.success) {
      router.push(`/resultados?type=flight&data=${encodeURIComponent(JSON.stringify(response))}`)
    }
  }
