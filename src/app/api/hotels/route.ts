...
export async function GET(request: NextRequest) {
  try {
    ...
    if (city) {
      baseQuery += ` AND LOWER(city) LIKE LOWER(${paramIndex})`
      params.push(`%${city}%`)
      paramIndex++
    }

    if (destination) {
      baseQuery += ` AND (LOWER(city) LIKE LOWER(${paramIndex}) OR LOWER(location) LIKE LOWER(${paramIndex}))`
    ...
    // Filtro por amenidades (usando JSONB)
    if (amenities && amenities.length > 0) {
      for (const amenity of amenities) {
        baseQuery += ` AND amenities->>${paramIndex} = 'true'`
        params.push(amenity)
        paramIndex++
      }
    }
