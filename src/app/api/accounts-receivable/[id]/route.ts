 ...
 * Obtener detalles de una cuenta por cobrar
 */
export async function GET(
  ...
  try {
    ...
    if (!account) {
      return NextResponse.json({
        success: false,
        error: 'Account receivable not found'
      }, { status: 404 })
    }
 ...
 * Registrar un pago a la cuenta por cobrar
 */
export async function PUT(
  ...
  try {
    ...
    if (newBalance < -0.01) { // Tolerancia para redondeo
      return NextResponse.json({
        success: false,
    ...
    const updatedAccount = await updateOne('accounts_receivable', parseInt(params.id), {
      balance: newBalance,
      status: newStatus,
      paid_at: paidAt,
      updated_at: new Date()
    ...
    await insertOne('payment_transactions', {
      account_receivable_id: parseInt(params.id),
...
export async function DELETE(
  ...
  try {
    ...
    // No permitir cancelar cuentas pagadas
    ...
    return NextResponse.json({
      success: true,
      data: updatedAccount,
      message: 'Account receivable cancelled successfully'
    })

  } catch (error) {
    console.error('Error cancelling account receivable:', error)
    return NextResponse.json({
      success: false,
      error: 'Failed to cancel account receivable',
      message: (error as Error).message
    }, { status: 500 })
  }
}
