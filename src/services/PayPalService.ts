...
function environment() {
  ...
  // Usar variable PAYPAL_MODE para determinar el ambiente
  // Por defecto usar sandbox si no está configurado o si el clientId empieza con 'A' (sandbox)
  const isProduction = process.env.PAYPAL_MODE === 'live'

  if (isProduction) {
    console.log('🟢 PayPal: Usando ambiente LIVE')
    return new paypal.core.LiveEnvironment(clientId, clientSecret)
  } else {
    console.log('🟡 PayPal: Usando ambiente SANDBOX')
