...
- **Flujo de Iniciar Sesión y Registro con Google (`login/page.tsx`, `registro/page.tsx`):**
  - Manejo de fallback cliente/servidor para la autenticación con Google.
  - Adición del botón de Google en el formulario de registro (`/registro`).
  - Detección de parámetros de error en URL (`?error=config_missing`, `?error=oauth_cancelled`, etc.) para mostrar mensajes descriptivos al usuario.
- **Módulo de Recuperación de Contraseña (`forgot-password/page.tsx`, `reset-password/page.tsx`):**
  - Implementación completa e interactiva del formulario de solicitud de recuperación de contraseña conectado con `POST /api/auth/forgot-password`.
  - Creación de la página `/reset-password` para validar el token enviado por correo (`GET /api/auth/reset-password?token=...`) y actualizar la contraseña mediante `POST /api/auth/reset-password`.
**Arquitectura Global de Intranet en 2 Secciones (Estilo ERPCubox) & Unificación de Layout**
- **Menú Vertical Adaptativo & Intranet Layout (`PortalSidebar.tsx`, `PortalIntranetLayout.tsx`):** Estructura unificada en 2 columnas estilo ERPCubox para todos los usuarios autenticados:
  - **Columna 1 (Izquierda):** Menú vertical colapsable adaptado automáticamente por rol (Cliente vs Agente/Staff).
  - **Columna 2 (Centro):** Área de trabajo principal dinámica donde cargan las vistas del portal (`/mis-reservas`, `/perfil`, `/facturacion`, `/operacion`, etc.).
- **Unificación de Páginas en Layout Persistente:** Reestructuración de vistas para compartir el mismo Header, Menú lateral y estética Marca Blanca.

### v2.436 - 27 de Julio de 2026 - 18:32 CST
**Portal Reestructura Fase 2: Menú Operación CRM, Lector SAT, Modal Idioma/Moneda & Marca Blanca Global**
- **Menú Operación & Sidebar CRM (`OperationSidebar.tsx`, `/operacion`):** Implementación del menú de Operación desplegable para roles `> cliente` con la navegación vertical idéntica a la Imagen 2 (*Dashboard, Catálogo Clientes, CRM, WhatsApp, Automatizaciones, Analytics*).
- **Modal de Selección de Idioma y Moneda (`LanguageCurrencyModal.tsx`, `LanguageCurrencyContext.tsx`):** Selector global `🌐 ES / MXN ∨` en el header para cambiar idioma (ES/EN) y 13 divisas (*MXN, USD, COP, CAD, EUR, ARS, BRL, CLP, CNY, GBP, JPY, PEN, UYU*) según la Imagen 4.
- **Lector de Constancia de Situación Fiscal SAT (`SATConstanciaUploader.tsx`, `SATParserService.ts`):** Componente para la lectura automática de la Hoja de Situación Fiscal en PDF/imagen, extrayendo RFC, Razón Social, Régimen Fiscal y CP para autocompletar formularios de facturación.
- **Unificación Look & Feel y Marca Blanca Global (`WhiteLabelContext.tsx`, `layout.tsx`):** Aplicación de estilos de la Landing Principal (Imagen 3) con soporte para parametrización por tenant y fallback predeterminado a AS Operadora.


### v2.434 - 27 de Julio de 2026 - 17:15 CST
**Reestructuración del Portal de Usuarios: Perfil, Reservas, Facturación CFDI y Conector PAC (Facturama)**
- **Migración BD 060:** Creación de tablas `linked_travelers` (compañeros de viaje tipo Expedia), `agency_billing_config`, `invoice_items`, `invoice_complements` (CFDI PPD 2.0) y extensión de `invoices`.
- **UserMenu Rediseñado (`UserMenu.tsx`):** Menú compacto de 3 opciones (Perfil, Mis reservas, Cerrar sesión) con tarjeta de datos de usuario. Accesos administrativos reubicados a `/perfil`.
- **Mi Perfil Completo (`perfil/page.tsx`):** 5 secciones colapsables (Datos personales, Seguridad, Usuarios vinculados con Expedia flow, Dispositivos vinculados reales con revocación remota, Módulos Admin para Staff).
- **Mis Reservas (`mis-reservas/page.tsx`):** UI según Mockup #4 con tabs por estado y cards mejoradas.
- **Detalle de Reserva (`reserva/[id]/page.tsx`):** Layout de 2 columnas (Mockups #5 y #6) con submenú acordeón "Modificar reserva" (nombre, fechas, call center).
- **Sistema de Facturación Backend & Frontend (`src/services/billing/`, `/facturacion`):** Conector PAC modular (`PACConnector.ts`, `FacturamaPACConnector.ts` basado en ERPCubox `facturama.js`), `InvoiceService.ts`, endpoints API `/api/billing/invoices/*`, lista de facturas con KPIs (Mockup #11) y Stepper de 4 pasos para timbrado/previsualización/descarga de CFDI (Mockup #8).
- **Centro de Ayuda & Prepara tu Viaje (`/ayuda`, `/ayuda/prepara-tu-viaje`):** Hub con chat 24/7, canales contextuales según fecha (Mockup #10) y guía de 8 temas navegables (Mockup #12).
